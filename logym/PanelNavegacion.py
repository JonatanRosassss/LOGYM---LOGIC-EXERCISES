...
class PanelNavegacionFiltros(wx.Panel):
    ...
    def inicializar_todos_los_componentes_del_panel_de_filtros(self):
        ...
        self.combo_desplegable_para_elegir_la_categoria.Bind(wx.EVT_COMBOBOX, self.filtrado_archivo)
        self.combo_desplegable_para_elegir_la_dificultad.Bind(wx.EVT_COMBOBOX, self.filtrado_archivo)
...
#cada vez que se detecta un ejercicio nuevo, pum, los inyecta
    def agregar_ejercicio_en_la_lista_visual(self, datos_en_un_diccionario_del_ejercicio: dict):
        ...
        #tiempo_del_ejercicio = datos_en_un_diccionario_del_ejercicio.get("configuracion", {}).get("tiempo", "5")
    ...
    def actualizar_temario(self, lista_con_las_categorias_a_cargar: list):
#antes de inyectar algo, elimina lo anterior
    ...
    def actualizar_opciones_filtros(self, categorias:list, dificultades:list):
        self.combo_desplegable_para_elegir_la_categoria.Clear()
        self.combo_desplegable_para_elegir_la_categoria.Append("Todas")
        for c in categorias:
            self.combo_desplegable_para_elegir_la_categoria.Append(c)
        self.combo_desplegable_para_elegir_la_categoria.SetSelection(0)

        self.combo_desplegable_para_elegir_la_dificultad.Clear()
        self.combo_desplegable_para_elegir_la_dificultad.Append("Todas")
        for d in dificultades:
            self.combo_desplegable_para_elegir_la_dificultad.Append(d)
        self.combo_desplegable_para_elegir_la_dificultad.SetSelection(0)

    def filtrado_archivo(self, event = None):
        import Utilidades

        categ_selec = self.combo_desplegable_para_elegir_la_categoria.GetStringSelection()
        dific_selec = self.combo_desplegable_para_elegir_la_dificultad.GetStringSelection()
        ejercicios = Utilidades.leer_ejercicios()
        self.limpiar()
        for e in ejercicios:
            categoria_ejercicio = e.get("configuracion", {}).get("categoria", "")
            dificultad_ejercicio = e.get("configuracion", {}).get("dificultad", "")
            cumple_categ = (categ_selec == "Todas" or categ_selec == categoria_ejercicio)
            cumple_dif  = (dific_selec == "Todas" or dific_selec == dificultad_ejercicio)

            if cumple_categ and cumple_dif:
                self.agregar_ejercicio_en_la_lista_visual(e)
    def limpiar(self):
        self.sizer_vertical_donde_se_van_acumulando_los_botones_de_ejercicios.Clear(True)
        self.ventana_con_scroll_para_los_ejercicios_disponibles.Layout()
        self.ventana_con_scroll_para_los_ejercicios_disponibles.FitInside()
